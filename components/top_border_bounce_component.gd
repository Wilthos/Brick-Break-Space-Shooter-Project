...
class_name TopBorderBounceComponent
extends Node
...
func _process(delta: float) -> void:
	# If the actor's y position is less than the top border plus the margin,
	# bounce off the top of the screen
	if actor.global_position.y < top_border + margin:
		# Prevent the actor for going past the border + the margin
		actor.global_position.y = top_border + margin
		...
		# (it's a bit counter intuitive but a wall on the top would have a wall face with a normal of UP)
